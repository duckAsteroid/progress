...
public abstract class AbstractProgressMonitor implements ProgressMonitor {
   ...
   */
  protected transient CopyOnWriteArrayList<AbstractProgressMonitor> children =
   ...
   */
  protected String unit = ""; // NOPMD - name clash with done() is ok
  ...
  public long worked(long amount, String status) {
    ...
    do {
      ...
      // if someone updated worked while we were computing the new value
      // it's not set - so try again...
    } while (!set);
  ...
  public void done() {
    if (!isDone()) {
      ...
      do {
        ...
        set = workDone.compareAndSet(worked, targetWorked);
        // if someone updated work while we were computing the new value & it was not set
        // try again...
      } while (!set);
      // log this update
  ...
  public ProgressMonitor newSubTask(String name, long work) {
    SubTaskProgressMonitor subTask =
        new SubTaskProgressMonitor(this, this.childId.getAndIncrement(), work, name);
