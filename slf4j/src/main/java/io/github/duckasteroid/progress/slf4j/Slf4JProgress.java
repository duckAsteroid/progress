...
public class Slf4JProgress implements ProgressMonitorListener {
   ...
   */
  private final transient Logger logger; // NOPMD - this is not OUR logger
  /**
   ...
   */
  private final transient Map<ProgressUpdateType, Level> levels;

  /**
   * Create new SLF4J wrapper.
   *
   * @param logger the logger to update
   * @param format the format of updates
   * @param level the level for updates
   */
  public Slf4JProgress(final Logger logger, final ProgressFormat format,
                       final Map<ProgressUpdateType, Level> level) {
    this.logger = logger;
    this.format = format;
